...
| 示例 | 类型 | 路径 | 内容 | 运行 |
|------|------|------|------|------|
| geek 蒸馏动效 | 静态 HTML + CSS | `geek-effects-demo.html` (+ `.css`) | 双层擦除按钮 `geek-btn-wipe`、背景像素画视差浮动 `geek-float-parallax`、错峰上浮 `geek-float-rise`、粒子网络 `geek-particle-bg`、滚动光带 `geek-marquee`、CRT 水波纹 `geek-crt-ripple` | 直接打开 HTML 预览 |
